"""
This file creates the Create, Read, Update, and Delete operations to our player data,
...
# Create the Template Web Application Interface for player routes to be applied 
...
def error_empty_table():
    return {"message": "No players found in this database. Add a player to get started."}

def error_player_does_not_exist(player_id):
    return {"message": f"Player ID {player_id} does not exist"}, 404

def player_successfully_removed(player_name):
    return {"message": f"Player {player_name} deleted successfully."}, 200 
...
def createPlayer():
    """
    Retrieve the body data and add the details of the player into the player database,
    this is the equivalent of POST in postgresql.
    """
    # Fetch the player information from the request body
    bodyData = request.get_json()
    
    # Create a new player object with the request body data as the attributes
    newPlayer = Player(
...
def getPlayers():
    ...
    # Return the search results if there are players in the player database, 
...
def getPlayer(player_id):
    ...
    # Return the search results if this player is in the player database, 
    # otherwise inform the user that the player does not exist.
    if queryData:
...
def updatePlayer(player_id):
    """
    Retrieve the body data and update the details of the player with the 
    matching ID in the player database, this is the equivalent of 
    ...
    # Update the player information in the players database if they exist
    if player:
        # Fetch the player information from the request body
        bodyData = request.get_json()

        # Update the player's details with these new changes, otherwise 
...
def deletePlayer(player_id):
    """
    Find the player with the matching ID in the player database and remove them.
