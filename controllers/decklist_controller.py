...
decklistsBp = Blueprint("cards", __name__, url_prefix = "/decklists")
...
def create_decklist():
    ...
    # rules implemented.
    addCardToDeck = Decklist(
        deck_id = bodyData.get("deck_id"),
        card_id = bodyData.get("card_id"),
...
def get_decklists():
    ...
    """
    # Select all the decklists from the database and the students that
    # are enrolled in these courses
    deck_id = request.args.get("deck_id", type = int)
    card_id = request.args.get("card_id", type = int)
    statement = db.select(Decklist)
    
    # Display decklists that exist
    if deck_id:
        statement = statement.where(Decklist.deck_id == deck_id)
    
    # Display students that exist in the decklist
    if card_id:
...
@decklistsBp.route("/<int:deck_id>&<int:card_id>", methods = ["DELETE"])
def delete_decklist(deck_id, card_id):
    """
    Find the decklist with the matching ID in the decklist database and remove it.
    This is the equivalent of DELETE in postgresql.
    """
    # Selects all the decklists from the database and filter the decklist with
    # matching ID
