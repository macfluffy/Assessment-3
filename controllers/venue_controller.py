"""
This file creates the Create, Read, Update, and Delete operations to our venue data,
...
# Create the Template Web Application Interface for venue routes to be applied 
...
def error_empty_table():
    return {"message": "No venues found in this database. Add a venue to get started."}

def error_venue_does_not_exist(venue_id):
    return {"message": f"Venue ID {venue_id} does not exist"}, 404

def venue_successfully_removed(venue_name):
    return {"message": f"The {venue_name} venue has been deleted successfully."}, 200 
...
def createVenue():
    """
    Retrieve the body data and add the details of the venue into the venue database,
    this is the equivalent of POST in postgresql.
    """
    # Fetch the venue information from the request body
    bodyData = request.get_json()
    
    # Create a new venue object with the request body data as the attributes
    newVenue = Venue(
        venue_name = bodyData.get("venue_name"),
        venue_address = bodyData.get("venue_address"),
        venue_number = bodyData.get("venue_number")
    )
...
def getVenues():
    ...
    # Return the search results if there are venues in the venue database, 
...
def getVenue(venue_id):
    ...
    """
    # Selects all the venues from the database and filter the venue with
    # matching ID
    ...
    # Return the search results if this venue is in the venue database, 
    # otherwise inform the user that the venue does not exist.
    if queryData:
        # Return the venue info in JSON format
        return jsonify(queryData)
    else:
        # Return an error message: Venue with this ID does not exist
...
def updateVenue(venue_id):
    """
    Retrieve the body data and update the details of the venue with the 
    matching ID in the venue database, this is the equivalent of 
    ...
    # Update the venue information in the venues database if they exist
    if venue:
        # Fetch the venue information from the request body
        bodyData = request.get_json()

        # Update the venue's details with these new changes, otherwise 
        # reuse the same information
        venue.venue_name = bodyData.get("venue_name", venue.venue_name)
        venue.venue_address = bodyData.get("venue_address", venue.venue_address)
        venue.venue_number = bodyData.get("venue_number", venue.venue_number)
...
def deleteVenue(venue_id):
    """
    Find the venue with the matching ID in the venue database and remove them.
